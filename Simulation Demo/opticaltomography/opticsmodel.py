...
from opticaltomography.opticsutil import genGrid, genPupil, propKernel, genZernikeAberration, genMTF
...
#         plt.figure()
#         plt.imshow(af.abs(self.pupil))
#         plt.figure()
#         plt.imshow(af.abs(self.field_f))
#         plt.figure()
#         plt.imshow(af.arg(self.field_f))
#         plt.pause(0.05)
        return field_pupil
...
class Defocus(Aperture):
    ...
    def __init__(self, shape, pixel_size, wavelength, na, RI_measure = 1.0, pad = True, **kwargs):
        ...
        self.pupil              = genPupil(self.shape, self.pixel_size, self.na, wavelength)
        self.pupilstop          = (fxlin**2 + fylin**2 <= af.max(fxlin)**2).as_type(af_complex_datatype)
        self.prop_kernel_phase  = 1.0j*2.0*np.pi*self.pupil*self.pupilstop*((RI_measure/wavelength)**2 - fxlin*af.conjg(fxlin) - fylin*af.conjg(fylin))**0.5
#         plt.figure()
...
class ScatteringModels:
    ...
    def _genSphericalIllumination(self, fx_illu, fy_illu):
        ...
        Amplitude_Source    = 10 
        illumination_xy     = af.exp(1.0j*2.0*np.pi/self.wavelength*\
                                     (((xlin-fx_illu*self.shape[0])*af.conjg(xlin-fx_illu*self.shape[0]))+\
                                       ((ylin-fy_illu*self.shape[1])*af.conjg(ylin-fy_illu*self.shape[1])))**0.5)/\
                                      (((xlin-fx_illu*self.shape[0])*af.conjg(xlin-fx_illu*self.shape[0]))+\
                                       ((ylin-fy_illu*self.shape[1])*af.conjg(ylin-fy_illu*self.shape[1])))**0.5
        ...
        illumination_xy[indCenterRow,indCenterCol] = 10 + 0.0j
        
#         field_phase_temp = af.exp(1.0j*af.arg(illumination_xy))
#         field_temp  = af.to_array(np.abs(illumination_xy))
#         for a in range(5):
...
#             field_phase_temp = field_phase_temp + af.shift(field_phase_temp, a+1, 0) +\
#                       af.shift(field_phase_temp, -1*(a+1), 0) +\
#                       af.shift(field_phase_temp, 0, (a+1)) +\
#                       af.shift(field_phase_temp, 0, -1*(a+1))
#         illumination_xy = field_temp / 25 * field_phase_temp / 25
#         illumination_xy = illumination_xy / 10
#         if fx_illu==0 and fy_illu==0:
#             illumination_xy[illumination_xy.shape[0]//2,illumination_xy.shape[0]//2] = 10 + 1.0j #the center pixel should be infinite for a point source, it is important to set it as a large value say 1000 or the field has artificial effect 
        
        #only on axis, without x,y shift
        #illumination_xy     = af.exp(1.0j*2.0*np.pi/self.wavelength*((xlin*af.conjg(xlin)) + (ylin*af.conjg(ylin)))**0.5)/((xlin*af.conjg(xlin))+(ylin*af.conjg(ylin)))**0.5
        
        #GaussianH = af.exp(-(((xlin-fx_illu*self.shape[0])*af.conjg(xlin-fx_illu*self.shape[0]))/0.1+\
                           # ((ylin-fy_illu*self.shape[1])*af.conjg(ylin-fy_illu*self.shape[1]))/0.1))
        #fr = af.fft2(illumination_xy)
        #fr2 = af.fft2(GaussianH)
        #m,n = fr.shape
        #illumination_xy = af.ifft2(fr*fr2)
    ...
    def _setIlluminationOnGrid(self, fx_illu, fy_illu):
        ...
        fx_illu_on_grid = np.round(fx_illu*self.shape[1]/self.pixel_size)*self.pixel_size/self.shape[1] #modified by Yi
        fy_illu_on_grid = np.round(fy_illu*self.shape[0]/self.pixel_size)*self.pixel_size/self.shape[0]
...
class MultiPhaseContrast(MultiTransmittance):
    ...
    def forward3D(self, contrast_obj, fx_illu, fy_illu, fz_illu_layer):
        field, fx_illu, fy_illu, fz_illu   = self._genSphericalIllumination(fx_illu, fy_illu)
        field3D                            = af.constant(0.0, self.shape[0], self.shape[1], self.shape[2], dtype = af_complex_datatype)
        ...
        if (type(contrast_obj).__module__ == np.__name__):
            ...
            flag_gpu_inout                 = True
        #Binning
        obj_af = self._binObject(phasecontrast_obj_af) #binning in z. if slice_binning_factor=1, obj_af=phasecontrast_obj_af=contrast_obj
        ...
        for layer in range(int(fz_illu_layer),self.shape[2]):
            field_layer_conj[:, :, layer]  = af.conjg(field3D[:, :, layer])
            field3D[:, :, layer]          *= obj_af[:, :, layer] #field interacts with the phantom.
            if layer < self.shape[2]-1:
    ...
    def forward(self, contrast_obj, fx_illu, fy_illu, fz_illu_layer):
        ...
        field                              = self._propagationInplace(field, 48 * self.slice_separation[0])
        ...
        for layer in range(int(fz_illu_layer),self.shape[2]):
            field_layer_conj[:, :, layer]  = af.conjg(field)
            field[:, :]                   *= obj_af[:, :, layer] #field interacts with the phantom.
            if layer < self.shape[2]-1:
                #self.slice_separation = self.pixel_size_z * np.ones((shape[2]-1,), dtype = np_float_datatype) #even speration
                field                      = self._propagationInplace(field, self.slice_separation[layer])
            #Yi's modify-------------------------------------------------------------------------------------------------
#             fieldPhase = af.exp(1.0j*af.arg(field))
#             field_temp  = af.to_array(np.abs(field))
#             for a in range(1):
#                 field_temp = field_temp + af.shift(field_temp, a+1, 0) +\
#                           af.shift(field_temp, -1*(a+1), 0) +\
#                           af.shift(field_temp, 0, (a+1)) +\
#                           af.shift(field_temp, 0, -1*(a+1))
#             field = field_temp / 10 * fieldPhase
        #------------------------------------------------------------------------------------------------------------
#         plt.figure()
...
#         plt.pause(0.05)        
        ...
        if self.focus_at_begin:
            #only for USAF target
            field                          = self._propagationInplace(field, self.distance_end_to_begin+375*0.48, adjoint = True)
            print(self.distance_end_to_begin)
        if self.focus_at_center:
            #propagate to volume center
            field                          = self._propagationInplace(field, self.distance_end_to_center, adjoint = True)
        return {'forward_scattered_field': field, 'cache': cache}
